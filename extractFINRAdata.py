...
def get_prices_and_volumes(path):
...
def extract_tba_prices(zf):
    ...
    for trading_day in pxtables_list:
        ...
        # check to see if filepath exists and append headers if they don't
        if os.path.isfile('csv/prices_tba.csv'):
            pass
        else:
            pd.DataFrame(df.columns).transpose().to_csv('csv/prices_tba.csv', header=False, index=False, mode='a')

        df.to_csv('csv/prices_tba.csv', header=False, index=False, mode='a')


def extract_cmo_prices(zf):
    price_fields = ['AVERAGE PRICE', 'WEIGHTED AVG. PRICE', 'AVG. PRICE BOTTOM 5 TRADES', '2ND QUARTILE PRICE',
                    '3RD QUARTILE PRICE', '4TH QUARTILE PRICE', 'AVG. PRICE TOP 5 TRADES', 'STANDARD DEVIATION',
                    'VOLUME OF TRADES (000\'S)', 'NUMBER OF TRADES']
    ...
    for trading_day in pxtables_list:
        ...
        # check to see if filepath exists and append headers if they don't
        if os.path.isfile('csv/prices_cmo.csv'):
            pass
        else:
            pd.DataFrame(df.columns).transpose().to_csv('csv/prices_cmo.csv', header=False, index=False, mode='a')

        df.to_csv('csv/prices_cmo.csv', header=False, index=False, mode='a')

def extract_mbs_prices(zf):
    # build namelist of specific files I need
    ...
    for trading_day in star_list:
        ...
        # check to see if filepath exists and append headers if they don't
        if os.path.isfile('csv/prices_mbsfixed.csv'):
            pass
        else:
            pd.DataFrame(df1.columns).transpose().to_csv('csv/prices_mbsfixed.csv', header=False, index=False, mode='a')

        if os.path.isfile('csv/prices_mbsfloating.csv'):
            pass
        else:
